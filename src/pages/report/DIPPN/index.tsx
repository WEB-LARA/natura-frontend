...
import { postDipn } from '@/services/natura/naturareport';

const ReportDIPPN: React.FC = () => {
  ...
  const handleFinish = async () => {
    ...
    if (report) {
      ...
      async function downloadPdfMe() {
        try {
          await postDipn(report!)
            .then((res) => res.blob())
            .then((response) => {
              const url = window.URL.createObjectURL(new Blob([response]));
              const link = document.createElement('a');
              link.href = url;
              link.setAttribute('download', 'ReportDIPPN.xlsx'); //or any other extension
  ...
  return (
    <>
      <PageHeader
        ...
        title="Laporan Daftar Intransit Penerima Penghasilan Natura"
        subTitle="Form DIPPN Natura untuk generate report"
      />
