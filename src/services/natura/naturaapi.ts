...
export async function fetchCarsApiHeader(
  ...
  return request<API.ResponseResult<API.CarsHeader[]>>('/api/v1/tamp-car-headers', {
    method: 'GET',
    params: {
      current: '1',
      pageSize: '10',
...
export async function getCarsApiHeader(id: string, options?: { [key: string]: any }) {
  return request<API.ResponseResult<API.CarsHeader>>(`/api/v1/tamp-car-headers/${id}`, {
    method: 'GET',
    ...(options || {}),
  });
}
...
export async function updateCarsApiHeader(
  ...
  return request<API.ResponseResult<any>>(`/api/v1/tamp-car-headers/${id}`, {
    method: 'PUT',
    data: body,
    ...(options || {}),
  });
